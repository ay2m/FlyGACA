...
async function recordFlightHour(
  ...
  const entry: FlightHour = { date, hours, flightType: flightType as any, aircraftType, instructor };
...
async function getAiracDueStatus(
  ...
  const banksDue = Object.entries(learner.progressByBankId)
    .filter(([_, prog]) => prog.airacStatus === 'due_soon' || prog.airacStatus === 'overdue')
    .map(([bankId]) => bankId);
...
describe('Learner data pipeline', () => {
  ...
  describe('flight-hour tracking', () => {
    ...
    it('marks currency expired after 90 days', async () => {
      const uid = 'pilot_003';
      const lastFlight = new Date('2026-01-01');
      const checkDate = new Date('2026-04-01'); // >90 days later
  ...
  describe('curriculum progress tracking', () => {
    ...
    it('maintains parity with iOS SwiftData schema', async () => {
      ...
      const profile = await ingestLearnerProfile(uid, 'student@example.com');
  ...
  describe('PDPL compliance — right to be forgotten', () => {
    ...
    it('preserves aggregated data (no PII)', async () => {
      ...
      const profile = await ingestLearnerProfile(uid, 'learner@example.com', 'Jane Doe');

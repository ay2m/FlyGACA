...
describe("isOverMonthlyQuota", () => {
  it("caps starter/growth at their monthly quota", () => {
    expect(isOverMonthlyQuota(4999, "starter")).toBe(false);
    expect(isOverMonthlyQuota(5000, "starter")).toBe(true);
    expect(isOverMonthlyQuota(24999, "growth")).toBe(false);
    expect(isOverMonthlyQuota(25000, "growth")).toBe(true);
  });
...
describe("remainingThisMonth", () => {
  it("counts down and floors at zero for capped tiers", () => {
    expect(remainingThisMonth(0, "starter")).toBe(5000);
    expect(remainingThisMonth(4990, "starter")).toBe(10);
...
describe("API_TIERS pricing (mirror guard for docs/strategy)", () => {
  it("holds the indicative list prices", () => {
    expect(API_TIERS.starter).toEqual({ monthlyQuota: 5000, priceSar: 499 });
    expect(API_TIERS.growth).toEqual({ monthlyQuota: 25000, priceSar: 1999 });
